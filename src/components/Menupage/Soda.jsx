    ...
    <ul className='p-4 grid gap-4 grid-cols-4 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5'>
      {itemsAttempSearch.map((sodaItem) => (
        <li
          ...
          key={sodaItem.id}>
            <Link to={`/desc/${sodaItem.id}`}><img src={sodaItem.img} alt={sodaItem.name} className='rounded w-[100%] object-cover cursor-pointer xs:h-[100px] sm:h-[150px] md:h-[180px] lg:h-[180px] xl:h-[180px]' /></Link>
